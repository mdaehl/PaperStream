...
from .base import ContentHandler, KeyedContentHandler
...
class IEEEContentHandler(KeyedContentHandler):
    ...
    def get_request_identifiers(self, paper_list: list[Paper]) -> list[list[str]]:
        ...
        if self.use_api:
            ...
            super().get_request_identifiers(paper_list)
    ...
    def _get_article_numbers_list(self, paper_list: list[Paper]) -> list[list[str]]:
        ...
        return list(
            map(lambda x: list(x), batched(article_numbers, self.api_max_records))
        )
    ...
    def _get_api_request_urls(self, paper_list: list[Paper]) -> list[str]:
        """Build request urls for the API, where multiple papers are grouped to reduce the number of requests.
    ...
    def get_paper_contents_from_request_content(
        ...
        if self.use_api:
            ...
            article_contents = sorted(
                article_contents, key=lambda x: content_ids.index(x["article_number"])
            )
            paper_data_list = [
